...
| Every phone reloads itself on every playbook run | The build is a hash of `ui.html`, so this means the file really is changing. A local edit under `src/player/` rebuilt into `dist/playstick-ui.html` is the usual reason; `git status` says |
